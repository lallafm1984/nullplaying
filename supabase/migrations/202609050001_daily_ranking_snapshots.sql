...
begin
  ...
  select s.snapshot_id, s.participant_count into strict v_snapshot_id, v_total
   ...
   where state.singleton;
  return query
    select r.rank_number, r.list_index, r.user_id, r.character_id, r.slot_id,
           r.display_name, r.hero_class, r.level, r.combat_power,
           r.achieved_at, r.updated_at, v_total
      from ranking_private.daily_leaderboard_rows as r
     where r.snapshot_id = v_snapshot_id
       and (r.list_index < least(greatest(coalesce(p_limit, 1000), 1), 1000)
...
-- scheduler attempts each minute so transient failures recover automatically;
-- the idempotent pointer check makes all but the first daily attempt constant-time.
