-- Runtime fix for shared-player validation and roster functions.
-- PostgreSQL LEAST/GREATEST are parser expressions and cannot be schema-qualified.
-- The original functions caught the resulting validator error as a false input result; this
-- migration replaces all three affected definitions and executes a valid-snapshot smoke check.

create or replace function shared_player_private.is_valid_snapshot(p_snapshot jsonb)
returns boolean
language plpgsql
immutable
strict
...
begin
  ...
  v_max_item_contribution := greatest(
    greatest(v_benchmark - 10, 0) + 30,
    (v_max_shop * 105 + 99) / 100
  ) + 11;
  v_min_power := greatest(least(v_legacy_power, v_guided_power) - 1, 1);
  v_max_power := greatest(v_legacy_power, v_guided_power) + v_max_item_contribution + 1;
...
create or replace function public.sync_public_player_snapshots(p_snapshots jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
  ...
  if exists (
    select 1
     ...
     where entry.level < saved.level
        or entry.level > least(
          10000::bigint,
          saved.level + greatest(
            1::bigint,
            pg_catalog.floor(
...
do $shared_player_validation_smoke$
declare
  v_sample jsonb := '{"character_id":"a1100000-0000-4000-8000-000000000001","display_name":"AQ-Validator","hero_class":"WARRIOR","level":10000,"combat_power":49996,"rules_version":1,"snapshot_version":1,"stats":{"strength":3350,"constitution":3350,"dexterity":3350,"intelligence":3350,"wisdom":3350,"charisma":3350,"max_health":200000,"max_mana":100000},"adventure_trait_ids":[]}'::jsonb;
begin
  if not shared_player_private.is_valid_snapshot(v_sample) then
    raise exception 'shared-player valid snapshot smoke check failed';
  end if;
end;
$shared_player_validation_smoke$;
