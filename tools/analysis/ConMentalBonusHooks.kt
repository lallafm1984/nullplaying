...
/** Planning-only replacement; not compiled into the Android application. */
object BalanceSimulationHooks {
    ...
    fun offlineHours(state: SimpleGameState): Double =
        8 + 2 * fraction(state.hero.stats.constitution).pow(.6) * (.15 + .85 * progress(state).pow(1.2))
    fun rawProbability(state: SimpleGameState, basePercent: Int): Double {
        // Convert before summation to avoid Long overflow. The weights apply to
        // the additional bonus; the engine's class-aptitude base is unchanged.
    ...
    fun saleBonus(state: SimpleGameState) = .2 * utilityRatio(state,state.hero.stats.charisma)
