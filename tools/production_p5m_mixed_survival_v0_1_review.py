...
def main()->int:
 ...
 for f in ("verificationTarget=android-emulator","avdName=alarmquest-qa","androidRelease=15","apiLevel=35","installResult=Success","pmClearResult=Success","launchState=COLD","coldTotalTimeMs=1019","topResumedActivity=com.alarmquest/.MainActivity","freshRosterEmpty=true","androidRuntimeFatalCount=0","engineTests=415","appTests=176","totalTests=591","liveSettlementEnabled=false"): ck("audit "+f.split("=")[0],f in audit,f)
 if args.with_emulator:
  d=run(["adb","devices","-l"]); ck("emulator online","emulator-5554" in d.stdout and " device " in d.stdout,d.stdout.strip()); b=run(["adb","-s","emulator-5554","shell","getprop","sys.boot_completed"]); ck("boot",b.stdout.strip()=="1",b.stdout.strip()); v=run(["adb","-s","emulator-5554","shell","dumpsys","package","com.alarmquest"]); ck("version","versionCode=1" in v.stdout and "versionName=0.1.0" in v.stdout,"0.1.0(1)"); f=run(["adb","-s","emulator-5554","shell","dumpsys","activity","activities"]); ck("focus","topResumedActivity" in f.stdout and "com.alarmquest/.MainActivity" in f.stdout,"MainActivity")
  ui=""
