...
import com.alarmquest.remote.maximumAcceptedRankingCombatPower
...
class PostgameJourneyTest {
    ...
    @Test
    fun `every class enters near day fifty five and reaches level one hundred around depth eighty seven`() {
        HeroClass.entries.forEach { heroClass ->
            ...
            assertTrue("$heroClass first labyrinth=$firstLabyrinthQuarter", firstLabyrinthQuarter in 217..226)
            assertTrue("$heroClass level 100=$levelOneHundredQuarter", levelOneHundredQuarter in 663..672)
    ...
    @Test
    fun `reference warrior reaches the labyrinth near day fifty five and depth eighty six near level one hundred`() {
        val game = newGame()

        engine.settleOffline(game, 52L * DAY_MILLIS)
        assertEquals(TaleKind.EPILOGUE, game.adventureTale.kind)

        engine.settleOffline(game, 55L * DAY_MILLIS)
        ...
        engine.settleOffline(game, 165L * DAY_MILLIS)
        assertTrue(game.hero.level < 100L)

        engine.settleOffline(game, 166L * DAY_MILLIS)
