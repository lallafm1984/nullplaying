...
/** V3 class milestones. V1/V2 and legacy V6 replays retain their original runtimes. */
object ArenaIdentityEngineV3 {
    ...
    fun validate(input: ArenaSupportInput) {
        ...
        require(snap.version==ARENA_IDENTITY_V3_RULES_VERSION && input.traits.isEmpty())
    ...
    private class Run(left: ArenaSupportInput,right: ArenaSupportInput,val seed: Long,val rules: ArenaTurnRules,
        ...
        fun result(status: ArenaRunStatus,winner: Fighter?)=ArenaSupportResult(status,winner?.id,turn,
            ...
            ARENA_IDENTITY_V3_RULES_VERSION,seed,rules)
        ...
        fun score(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            ...
            if(!s.support) {
                ...
                return ((hurt+status+classUtility)/s.actionTurns+lethal)*completion-(if(s.id==BASIC) 0.0 else s.mp*.10)
            }
            ...
            return when(s.kind) {
                ...
                "VIGILANCE" -> vigilanceValue(f,t,s,enemyCast ?: availableEnemy.maxByOrNull {
                    attackPower(t,it)*it.damagePercent/100.0/it.actionTurns
                } ?: basic)
            ...
            }+classSupportUtility(f,s)-s.mp*.10
        }
        fun vigilanceValue(f: Fighter,t: Fighter,s: ArenaIdentitySkill,threat: ArenaIdentitySkill): Double {
            if(threat.support) return 0.0
            val hit=hitChance(t,f,threat)
            val checks=min(s.charges.toDouble(),s.duration.toDouble()/threat.actionTurns*hit)
            ...
            // Count only the skill's limited direct-hit checks, discounted for an uncertain future.
            return min(f.hp,raw*checks*extra*(1-mitigation(t,f)))*.90
        }
