...
/** V4 mana budgets and defensive planning; historical runtimes remain frozen. */
object ArenaIdentityEngineV4 {
    ...
    val attackKinds=setOf("accuracy","followup_damage","followup_accuracy","shield_extra","weaken_damage",
        ...
        "poison_bonus","opening_bonus","evasion_reduction","burn_release","guarded_bonus",
        "bleed_bonus","burn_bonus","consecutive_bonus","evasion_on_hit","mana_return")
    val supportKinds=damageBuffs+guards+cleanses+controls+pierces+coatings+setOf(
        ...
        "EVASION","POISON_ACCELERATE","SHIELD","LOW_SHIELD","MIRROR","STABILIZE","STATUS_GUARD","TRUTH","SANCTUARY","LIFESTEAL","VIGILANCE")

    fun validate(input: ArenaSupportInput) {
        val snap=requireNotNull(input.identity)
        require(snap.version==ARENA_IDENTITY_V4_RULES_VERSION && input.traits.isEmpty())
    ...
    private class Fighter(val input: ArenaSupportInput) {
        ...
        var lastAttackHit=false
        var dotDamageThisTurn=0.0
    ...
    private class Run(left: ArenaSupportInput,right: ArenaSupportInput,val seed: Long,val rules: ArenaTurnRules,
        ...
        fun result(status: ArenaRunStatus,winner: Fighter?)=ArenaSupportResult(status,winner?.id,turn,
            ...
            ARENA_IDENTITY_V4_RULES_VERSION,seed,rules)
        ...
        fun run(): ArenaSupportResult {
            ...
            for(t in 1..rules.safetyTurnLimit) {
                turn=t
                fighters.forEach { it.dotDamageThisTurn=0.0 }
        ...
        fun manaPenalty(f: Fighter,s: ArenaIdentitySkill): Double {
            // Spend freely while funded; prefer efficient follow-ups as the last third runs low.
            // This only orders legal actions: no extra regeneration or artificial MP deductions.
            val scarcity=(1-f.mp/(f.stats.mp*1000.0*.35)).coerceIn(0.0,1.0)
            return s.mp*(.10+.55*scarcity)
        }
        fun score(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            val a=f.stats.attack
            if(!s.support) {
                val activeBonus=f.buffs.values.filter { matches(it,s) && it.expires>=turn+s.actionTurns-1 }.sumOf { b ->
                    when(b.skill.kind) {
                ...
                val raw=attackPower(f,s)*s.damagePercent/100.0*(1+(activeBonus+conditionalDamage(f,t,s)).coerceAtMost(50.0)/100)
                ...
                val classUtility=(s.n("bleed_on_hit")+s.n("poison_on_hit")+s.n("burn_on_hit"))*a/100*2*hit+
                    min(f.stats.hp-f.hp,f.stats.hp*s.n("hit_heal")/100)+f.stats.hp*s.n("hit_shield")/100*.5
                val status=when(s.kind) {
                    ...
                    "evasion_reduction" -> evasionReductionValue(f,t,s)
                    "followup_accuracy" -> followupAccuracyValue(f,t,s)
                    "followup_damage" -> followupDamageValue(f,t,s)
                    "burn_release" -> burnToRelease(f,t)?.let { min(t.stats.hp*.05,remainingBurn(it)*s.magnitude/100) } ?: 0.0
                    "self_cleanse" -> cleanseValue(f)
                    "evasion_on_hit" -> (t.pressure+t.stats.attack)*s.magnitude/100
                    "mana_return" -> if(f.mp<f.stats.mp*500) a*s.magnitude/20 else 0.0
                    else -> 0.0
                }
                ...
                return ((hurt+status+classUtility)/s.actionTurns+lethal)*completion-(if(s.id==BASIC) 0.0 else manaPenalty(f,s)/s.actionTurns)
            }
            ...
            return when(s.kind) {
                ...
                "VIGILANCE" -> vigilanceValue(f,t,s)
            ...
            }+classSupportUtility(f,s)-manaPenalty(f,s)
        }
        fun vigilanceValue(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            // Forecast the entire active window, including attacks after an enemy support cast.
            // Only public state is used: no future rolls or the opponent's simultaneous choice.
            val ready=t.ready.toMutableMap()
            var mana=t.mp/1000.0
            var elapsed=t.casting?.remaining ?: 0
            var threat=t.casting?.skill
            var checks=s.charges.toDouble()
            var prevented=0.0
            val horizon=min(s.duration.toDouble(),t.hp/f.pressure.coerceAtLeast(1.0)).coerceAtLeast(0.0)
            while(elapsed<=horizon && checks>0) {
                if(threat==null) {
                    val candidates=t.skills.filter { !it.support && it.mp<=mana &&
                        (ready[it.id] ?: 0)<=turn+elapsed && it.earliestTurn<=turn+elapsed && !(it.once && it.id in t.used) }+basic
                    threat=candidates.maxByOrNull { attackPower(t,it)*it.damagePercent/100.0/it.actionTurns } ?: basic
                    elapsed+=threat.actionTurns
                    mana-=threat.mp
                }
                if(elapsed>horizon) break
                if(!threat.support) {
                    val hit=hitChance(t,f,threat)
                    val raw=min(attackPower(t,threat)*threat.damagePercent/100.0,
                        f.stats.hp*threat.n("damage_hp_cap_percent",100.0)/100)
                    val extra=(evasion(f,t,threat,vigilanceOverride=magnitude(s))-evasion(f,t,threat)).coerceAtLeast(0.0)
                    prevented+=raw*min(checks,hit)*extra*(1-mitigation(t,f))
                    checks-=hit
                }
                ready[threat.id]=if(threat.once) Int.MAX_VALUE else turn+elapsed+threat.cooldown+1
                mana+=threat.actionTurns
                threat=null
            }
            // At the opening, protect the following attack sequence while HP and MP permit it.
            // The forecast still must be worthwhile; finishing a weak opponent wins the comparison.
            val followup=f.skills.filter { !it.support }.minOfOrNull { it.mp } ?: 0
            val preparation=if(turn<=3 && f.hp>f.stats.hp*.6 && f.mp/1000>=s.mp+followup) 1.35 else 1.0
            return min(f.hp,prevented)*preparation
        }
        fun evasionReductionValue(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            val current=t.debuffs.filter { it.kind=="evasion" }.maxByOrNull { it.value }
            val old=current?.value ?: 0.0
            val available=evasion(t,f,basic,ignoreReduction=true)*100
            val next=min(s.magnitude,available)
            val prior=min(old,available)
            val remaining=current?.let { (it.expires-turn).coerceAtLeast(0) } ?: 0
            // Only the increase and genuinely extended turns help future attacks.
            val gain=(next-prior).coerceAtLeast(0.0)*s.duration+
                min(next,prior)*(s.duration-remaining).coerceAtLeast(0)
            return f.stats.attack*gain/100*1.4
        }
        fun followupAccuracyValue(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            val active=f.buffs["followup_accuracy"]?.takeIf { it.expires>=turn+2 }?.value ?: 0.0
            val extra=(s.magnitude-active).coerceAtLeast(0.0)/100
            val room=(.97-hitChance(f,t,basic)).coerceAtLeast(0.0)
            val uses=min(s.charges,max(1,s.duration/2))
            return f.stats.attack*1.4*min(extra,room)*uses*(1-evasion(t,f,basic))
        }
        fun followupDamageValue(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            val active=f.buffs["followup_damage"]?.takeIf { it.expires>=turn+2 }?.value ?: 0.0
            val extra=(s.magnitude-active).coerceAtLeast(0.0)/100
            val uses=min(s.charges,max(1,s.duration/2))
            return f.stats.attack*extra*uses*hitChance(f,t,basic)*(1-evasion(t,f,basic))
        }
        fun classSupportUtility(f: Fighter,s: ArenaIdentitySkill): Double =
            (s.n("next_bleed")+s.n("next_poison")+s.n("next_burn"))*f.stats.attack/100*1.5+
            min(f.stats.hp-f.hp,f.stats.hp*s.n("cast_heal")/100)+f.stats.hp*s.n("cast_shield")/100*.5
        fun cleanseValue(f: Fighter): Double {
            val d=f.debuffs.maxByOrNull { it.value } ?: return 0.0
            val remaining=(d.expires-turn+1).coerceAtLeast(0)
            return when(d.kind) {
                "poison","bleed","burn" -> d.value*remaining
                "accuracy","damage","evasion" -> f.stats.attack*1.4*d.value/100*remaining
                "healing" -> if(f.hp<f.stats.hp*.5) f.stats.hp*.08*d.value/100 else 0.0
                else -> 0.0
            }
        }
        ...
        fun control(f: Fighter,t: Fighter,s: ArenaIdentitySkill,base: Double,mode: String) {
            ...
            if(s.n("control_refund")>0) changeMp(f,s.n("control_refund").toInt()*1000,s)
            if(s.n("control_heal")>0) heal(f,s,f.stats.hp*s.n("control_heal")/100)
            if(s.n("control_shield")>0) addShield(f,s.copy(duration=2),f.stats.hp*s.n("control_shield")/100)
        ...
        fun removalBonus(f: Fighter,s: ArenaIdentitySkill,success: Boolean) {
            if(!success) return
            if(s.n("removal_damage")>0) addBuff(f,s,s.n("removal_damage"),"followup_damage",1,2)
            if(s.n("removal_ignore")>0) addBuff(f,s,s.n("removal_ignore"),"milestone_ignore",1,5)
            if(s.n("removal_poison")>0) addBuff(f,s,s.n("removal_poison"),"milestone_poison",1,5)
            if(s.n("removal_refund")>0) changeMp(f,s.n("removal_refund").toInt()*1000,s)
            if(s.n("removal_heal")>0) heal(f,s,f.stats.hp*s.n("removal_heal")/100)
            if(s.n("removal_shield")>0) addShield(f,s.copy(duration=2),f.stats.hp*s.n("removal_shield")/100)
        }
        fun directHeal(f: Fighter,s: ArenaIdentitySkill,amount: Double) {
            val before=f.hp
            heal(f,s,amount)
            if(f.hp>before) {
                if(s.n("heal_guard")>0) addBuff(f,s,s.n("heal_guard"),"milestone_guard",0,2)
        ...
        fun drain(f: Fighter,t: Fighter,s: ArenaIdentitySkill) {
            ...
            if(t.mp<before && s.n("drain_refund")>0) changeMp(f,s.n("drain_refund").toInt()*1000,s)
        }
        fun support(f: Fighter,t: Fighter,s: ArenaIdentitySkill) {
            ...
            classAfterSupport(f,s)
        }
        fun classAfterSupport(f: Fighter,s: ArenaIdentitySkill) {
            for((key,kind) in listOf("next_bleed" to "milestone_bleed", "next_poison" to "milestone_poison",
                "next_burn" to "milestone_burn", "next_ignore" to "milestone_ignore"))
                if(s.n(key)>0) addBuff(f,s,s.n(key),kind,1,5)
            if(s.n("cast_evasion")>0) addBuff(f,s,s.n("cast_evasion"),"milestone_evasion",0,2)
            if(s.n("cast_heal")>0) heal(f,s,f.stats.hp*s.n("cast_heal")/100)
            if(s.n("cast_shield")>0) addShield(f,s.copy(duration=2),f.stats.hp*s.n("cast_shield")/100)
        }
        // Today's tick already occurred before action selection. Only unspent future ticks count.
        fun remainingBurn(d: Debuff)=d.value*(d.expires-turn).coerceAtLeast(0)
        fun burnToRelease(f: Fighter,t: Fighter)=t.debuffs.filter {
            it.kind=="burn" && it.source==f.id && it.expires>turn
        }.maxByOrNull(::remainingBurn)
        fun releaseBurn(f: Fighter,t: Fighter,s: ArenaIdentitySkill,directDamage: Double) {
            val burn=burnToRelease(f,t) ?: return
            val cap=min(t.stats.hp*.05,min(t.stats.hp*.08-t.dotDamageThisTurn,
                t.stats.hp*s.n("damage_hp_cap_percent",100.0)/100-directDamage)).coerceAtLeast(0.0)
            if(cap<1) return
            t.debuffs.remove(burn)
            damage(f,t,s,remainingBurn(burn)*s.magnitude/100*(1-mitigation(f,t)),cap,"burn")
            if(s.n("detonation_refund")>0) changeMp(f,s.n("detonation_refund").toInt()*1000,s)
        }
        fun extendPoison(f: Fighter,t: Fighter,s: ArenaIdentitySkill) {
            if(s.n("poison_extend")<=0) return
            val old=t.debuffs.filter { it.kind=="poison" && it.source==f.id && it.expires<it.starts+3 }
                .maxByOrNull { it.value } ?: return
            val extra=old.copy(expires=min(old.starts+3,old.expires+s.n("poison_extend").toInt()))
            t.debuffs[t.debuffs.indexOf(old)]=extra
            emit(Event.STATUS_APPLIED,f,t,s,expires=extra.expires,reason="poison")
        }
        fun conditionalDamage(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double = (when(s.kind) {
            "poison_bonus" -> if(t.debuffs.any { it.kind=="poison" && it.source==f.id }) s.magnitude else 0.0
            "bleed_bonus" -> if(t.debuffs.any { it.kind=="bleed" && it.source==f.id }) s.magnitude else 0.0
            "burn_bonus" -> if(t.debuffs.any { it.kind=="burn" && it.source==f.id }) s.magnitude else 0.0
            "consecutive_bonus" -> if(f.lastAttackHit) s.magnitude else 0.0
            "opening_bonus" -> if(t.hp>=t.stats.hp*.7) s.magnitude else 0.0
            "guarded_bonus" -> if(f.shield>0 || f.buffs.keys.any { it in guards }) s.magnitude else 0.0
            ...
            (if(turn-f.lastHit<=2) s.n("counter_damage") else 0.0)+
            (if(f.hp<=f.stats.hp*.5) s.n("bloodied_damage") else 0.0)+
            (if(t.hp>=t.stats.hp*.7) s.n("opening_damage") else 0.0)+
            (if(f.lastAttackHit) s.n("consecutive_hit_damage") else 0.0)+
            (if(f.mp>=f.stats.mp*500) s.n("high_mana_damage") else 0.0)+
            (if(turn-f.lastHealTurn<=3) s.n("recent_heal_damage") else 0.0)+
            (if(f.shield>0 || f.buffs.keys.any { it in guards }) s.n("guarded_damage") else 0.0)
        fun matches(b: Buff,s: ArenaIdentitySkill)=when(b.skill.kind) {
            ...
            "SKILL_SHATTER","STABILIZE","milestone_burn" -> s.id!=BASIC
            else -> true
        }
        ...
        fun evasion(t: Fighter,f: Fighter,s: ArenaIdentitySkill,ignoreReduction: Boolean=false,
            vigilanceOverride: Double?=null): Double {
            val native=(t.stats.baseEvade+.04*ArenaIdentityFormula.edge(t.stats.evade,f.stats.aim)).coerceIn(.02,.12)
            val ignored=maxOf(if(s.capstone=="native_evasion_ignore") s.n("capstone_value") else 0.0,
                s.n("native_ignore"),f.buffs["milestone_ignore"]?.value ?: 0.0)/100
            ...
            val vigilance=vigilanceOverride ?: t.buffs["VIGILANCE"]?.value ?: 0.0
            val alert=if(vigilance>0) (vigilance/100+.04*ArenaIdentityFormula.edge(
                .6*t.input.fighter.stats.dexterity+.4*t.input.fighter.stats.wisdom,f.stats.aim)).coerceIn(.16,.30) else 0.0
            val reduction=if(ignoreReduction) 0.0 else (t.debuffs.filter { it.kind=="evasion" }.maxOfOrNull { it.value } ?: 0.0)/100
            val extra=((t.buffs["milestone_evasion"]?.value ?: 0.0)+(t.buffs["skill_evasion"]?.value ?: 0.0))/100
            return (1-(1-(native-ignored).coerceAtLeast(0.0))*(1-max(active,alert))+extra-reduction).coerceIn(0.0,.35)
        }
        ...
        fun attack(f: Fighter,t: Fighter,s: ArenaIdentitySkill) {
            ...
            f.lastAttackHit=false
            if(s.kind=="after_miss_bonus" && condition>0) f.lastMiss=-100
            buffs.filter { it.charges>0 && it.skill.kind !in setOf("MIRROR","EVASION","STATUS_GUARD","BASIC_GUARD","SKILL_GUARD","GRACE","DAMAGE_CAP","VIGILANCE") }
            ...
            val evadeRoll=random(f,"evade:${s.id}")
            val dodge=evadeRoll<evade
            val vigilanceDodge=dodge && "VIGILANCE" in t.buffs && evadeRoll>=evasion(t,f,s,vigilanceOverride=0.0)
            if("EVASION" in t.buffs) consume(t,"EVASION")
            if("VIGILANCE" in t.buffs) consume(t,"VIGILANCE")
            if(dodge) { f.lastMiss=turn;t.enemyMiss=turn;emit(Event.ATTACK_EVADED,t,f,s,reason=if(vigilanceDodge) "vigilance" else null);return }
            ...
            when(s.kind) {
                ...
                "evasion_reduction" -> debuff(f,t,s,"evasion",s.magnitude,s.duration)
                "burn_release" -> releaseBurn(f,t,s,dealt)
                "evasion_on_hit" -> addBuff(f,s,s.magnitude,"skill_evasion",0,s.duration)
                "mana_return" -> changeMp(f,s.magnitude.roundToInt()*1000,s)
            ...
            for(b in buffs) when(b.skill.kind) {
                ...
                "milestone_poison","milestone_bleed","milestone_burn" -> debuff(f,t,b.skill,b.skill.kind.removePrefix("milestone_"),attackPower(f,s)*b.value/100)
            ...
            if(t.hp<=0) return
            for((key,kind) in listOf("bleed_on_hit" to "bleed","poison_on_hit" to "poison","burn_on_hit" to "burn"))
                if(s.n(key)>0) debuff(f,t,s,kind,attackPower(f,s)*s.n(key)/100,2)
            if(s.n("hit_evasion")>0) addBuff(f,s,s.n("hit_evasion"),"milestone_evasion",0,2)
            if(s.n("next_ignore_on_hit")>0) addBuff(f,s,s.n("next_ignore_on_hit"),"milestone_ignore",1,5)
            if(s.n("next_basic_on_hit")>0) addBuff(f,s,s.n("next_basic_on_hit"),"next_basic_damage",1,3)
            if(s.n("hit_heal")>0) heal(f,s,f.stats.hp*s.n("hit_heal")/100)
            if(s.n("hit_shield")>0) addShield(f,s.copy(duration=2),f.stats.hp*s.n("hit_shield")/100)
            if(s.n("hit_mana_refund")>0) changeMp(f,s.n("hit_mana_refund").toInt()*1000,s)
        ...
        fun damage(f: Fighter,t: Fighter,s: ArenaIdentitySkill,amount: Double,cap: Double,
            ...
            if(dot!=null) t.dotDamageThisTurn+=before-t.hp
