...
/** V5 lighter defensive effects with retained opening use; published V4 stays frozen. */
object ArenaIdentityEngine {
    ...
    fun validate(input: ArenaSupportInput) {
        ...
        require(snap.version==ARENA_IDENTITY_RULES_VERSION && input.traits.isEmpty())
    ...
    private class Run(left: ArenaSupportInput,right: ArenaSupportInput,val seed: Long,val rules: ArenaTurnRules,
        ...
        fun result(status: ArenaRunStatus,winner: Fighter?)=ArenaSupportResult(status,winner?.id,turn,
            ...
            ARENA_IDENTITY_RULES_VERSION,seed,rules)
        ...
        fun vigilanceValue(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            ...
            val preparation=if(turn<=3 && f.hp>f.stats.hp*.6 && f.mp/1000>=s.mp+followup) 1.55 else 1.0
