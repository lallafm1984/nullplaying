...
/** V1 deterministic rules. Legacy ArenaSupportTurnEngine remains the V6 replay implementation. */
internal object ArenaIdentityEngineV1 {
    ...
    private val guards=setOf("IRON","WRIST","RESTRAINT","BASIC_GUARD","SKILL_GUARD","GRACE")
    ...
    fun validate(input: ArenaSupportInput) {
        ...
        require(snap.version==ARENA_IDENTITY_V1_RULES_VERSION && input.traits.isEmpty())
    ...
    private class Run(left: ArenaSupportInput,right: ArenaSupportInput,val seed: Long,val rules: ArenaTurnRules,
        ...
        fun result(status: ArenaRunStatus,winner: Fighter?)=ArenaSupportResult(status,winner?.id,turn,
            ...
            ARENA_IDENTITY_V1_RULES_VERSION,seed,rules)
        ...
        fun eligible(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Boolean {
            ...
            return when(s.kind) {
                ...
                "CLEANSE","SANCTUARY" -> hasNegative(f)
                ...
                "HEAL_TRACK" -> t.lastHealTurn>f.consumedHeal && turn-t.lastHealTurn<=2 && t.lastHeal>0
                "MP_DRAIN" -> t.mp>=5000
        ...
        fun score(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double {
            ...
            if(!s.support) {
                ...
                val status=when(s.kind) {
                    "burn_tick","poison_tick","bleed_tick" -> attackPower(f,s)*s.magnitude/100*2*hit
            ...
            return when(s.kind) {
                ...
                "REGEN" -> min(f.stats.hp-f.hp,f.stats.hp*v*3)*need
                ...
                "HEAL_TRACK" -> min(t.lastHeal*v,a*.25)
        ...
        fun control(f: Fighter,t: Fighter,s: ArenaIdentitySkill,base: Double,mode: String) {
            ...
            val chance=(base/100+.10*ArenaIdentityFormula.edge(f.stats.aim,t.stats.resistance)-resist/100).coerceIn(.30,.75)
        ...
        fun support(f: Fighter,t: Fighter,s: ArenaIdentitySkill) {
            ...
            when(s.kind) {
                "HEAL","BANDAGE","LAY_HANDS" -> heal(f,s,f.stats.hp*magnitude(s)/100)
                "SHIELD","LOW_SHIELD" -> addShield(f,s,f.stats.hp*magnitude(s)/100)
                "CLEANSE","CLEANSE_ACCURACY" -> if(cleanse(f,s,s.kind=="CLEANSE_ACCURACY")) addBuff(f,s,6.0,"followup_accuracy",1,2)
                "DISPEL","JUDGMENT","SEAL","REVEAL" -> if(dispel(t,s,s.kind=="REVEAL")) addBuff(f,s,6.0,"followup_accuracy",1,2)
                ...
                "POISON_ACCELERATE" -> t.debuffs.firstOrNull { it.kind=="poison" && it.source==f.id && it.expires>turn }?.let { old ->
                    if(t.debuffs.count { it.kind=="poison" }<3) t.debuffs+=old.copy(starts=turn+1,firstTickMultiplier=1.2)
                }
        ...
        fun conditionalDamage(f: Fighter,t: Fighter,s: ArenaIdentitySkill): Double = when(s.kind) {
            ...
            "after_miss_bonus" -> if(turn-f.lastMiss<=2) s.magnitude else 0.0
            "execute_bonus" -> if(t.hp<=t.stats.hp*.35) s.magnitude else 0.0
        ...
        fun attack(f: Fighter,t: Fighter,s: ArenaIdentitySkill) {
            ...
            if(mirror!=null) {
                consume(t,"MIRROR")
                if(random(f,"mirror:${s.id}")<.25*(1-truth/100)) { f.lastMiss=turn;t.enemyMiss=turn;emit(Event.ATTACK_EVADED,t,f,s,reason="mirror");return }
            }
            ...
            buffs.firstOrNull { it.skill.kind=="HEAL_TRACK" }?.let { b ->
                if(t.lastHealTurn>f.consumedHeal && turn-t.lastHealTurn<=2) {
                    raw+=min(t.lastHeal*b.value/100,attackPower(f,s)*.25)
            ...
            when(s.kind) {
                "followup_damage","followup_accuracy" -> addBuff(f,s,s.magnitude,s.kind,1,2)
                "weaken_damage" -> debuff(f,t,s,"damage",s.magnitude)
                "slow_accuracy" -> debuff(f,t,s,"accuracy",s.magnitude)
                "heal_reduction" -> debuff(f,t,s,"healing",s.magnitude)
                "poison_tick","bleed_tick","burn_tick" -> debuff(f,t,s,s.kind.substringBefore('_'),attackPower(f,s)*s.magnitude/100)
                ...
                "heal_on_hit_hp" -> heal(f,s,f.stats.hp*s.magnitude/100)
                "shield_on_hit_hp" -> addShield(f,s,f.stats.hp*s.magnitude/100)
                "dispel" -> dispel(t,s)
                "self_cleanse" -> cleanse(f,s)
                "remove_illusion" -> t.buffs["MIRROR"]?.let { consume(t,"MIRROR") }
                "mp_drain" -> changeMp(t,-s.magnitude.roundToInt()*1000,s,true)
