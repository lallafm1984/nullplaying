...
internal data class AdventureTaleDefinition(
    ...
    val nextId: String?,
    val openingSlideTemplates: List<String> = emptyList(),
)
...
internal object AdventureTaleCatalog {
    ...
    const val REPEAT_ACT_TARGET = 1_000L

    /** Short -> medium -> tension -> relief -> boss. */
    internal val MAIN_ACT_RHYTHM_WEIGHTS = listOf(3L, 5L, 6L, 4L, 7L)
    ...
    private val COMPLETION_BY_TITLE = mapOf(
        ...
        "잠긴 수로" to "{minorEnemyObject} 몰아내고 버려진 주조장으로 통하는 수로를 다시 열었다.",
        "버려진 주조장" to "떼어 낸 청동 자국을 비교해 누군가 새벽종을 해체했음을 밝혀냈다.",
        "첫 번째 새벽종" to "첫 번째 새벽종과 그 표면에 새겨진 오르반의 이름을 확보했다.",
        // 3장
        ...
        // 4장
        "벽뿐인 예배당" to "예배당 벽화에서 첫 새벽종을 울리면 숨겨진 길이 열린다는 문구를 확인했다.",
        "종소리가 여는 길" to "첫 번째 새벽종을 울려 비밀 계단을 열고 오르반이 숨은 지하 예배실에 도착했다.",
        "세 개의 새벽종" to "세 종이 재의 문을 여는 무기가 아니라 문을 잠그는 열쇠임을 알아냈다.",
        "침묵의 찬가" to "오르반의 침묵 찬가에서 남은 두 새벽종이 모두 도난당했다는 사실을 들었다.",
        ...
        "두 번째 새벽종" to "전복된 수레에서 두 번째 새벽종을 되찾았지만 마지막 수레를 놓쳤다.",
        ...
        "마지막 새벽종" to "베른이 마지막 새벽종을 울려 재의 왕을 깨웠다.",
        // 11장
        "꺼진 하늘" to "세 번째 종소리와 함께 새벽이 사라지고 재의 왕이 깨어났다.",
        "되돌아온 길들" to "여정 동안 열어 둔 수로·지하로·성벽길을 하나의 피난로로 연결했다.",
        "세 종의 답" to "오르반이 두 종소리를 한 박자로 묶어 {heroSubject} 마지막 종에 닿을 틈을 만들었다.",
        "재의 왕" to "{heroSubject} 재의 왕 갑옷을 갈라 그 안의 잊힌 국경 사람들의 이름을 해방했다.",
        "베른의 마지막 명령" to "베른이 문을 붙들고 모든 이름을 남긴 첫 철수 명령으로 사람들을 살렸다.",
        // 12장
        "되찾은 세 새벽종" to "흩어진 세 새벽종을 모으고 오르반이 상처 난 세 종의 음을 맞춰 냈다.",
        "세 탑의 불빛" to "로웬·니아·{heroSubject} 성벽·지하로·성채 종탑에서 동시 울림을 준비했다.",
    ...
    val mainTales: List<AdventureTaleDefinition> = listOf(
        chapter(
            ...
            "{heroTopic} 로웬과 순찰대의 생존자를 구했다. 순찰대를 괴물에게 몰아넣은 자는 국경 안에 있었고, 로웬의 지도는 폐쇄된 우물을 가리켰다.",
            "남은 단서: 폐쇄된 우물과 세 개의 검은 원.",
            act("empty_barracks", "빈 막사", "동이 텄는데 순찰대의 침상은 차갑고, 식지 않은 등불만 주인을 기다렸다."),
            act("inward_tracks", "거꾸로 난 발자국", "{heroTopic} 국경 밖이 아니라 성벽 안쪽에서 시작된 발자국을 찾아냈다."),
        ...
        chapter(
            ...
            "{heroTopic} 우물 아래 수로와 버려진 주조장을 지나 첫 번째 새벽종을 찾았다. 종 표면에는 순찰대 지도와 같은 검은 원과 오르반의 이름이 새겨져 있었다.",
            "남은 단서: 첫 번째 새벽종과 종지기 오르반.",
            ...
            act("locked_channel", "잠긴 수로", "{heroTopic} 오래된 수로를 막은 {minorEnemyObject} 몰아내고 벽 너머의 바람을 들었다."),
            act("abandoned_foundry", "버려진 주조장", "수로 끝 주조장에는 종을 만들던 틀과 억지로 뜯어낸 청동 조각이 남아 있었다."),
            act("first_clapper", "첫 번째 새벽종", "진흙 속에서 찾은 새벽종 표면에는 순찰대 지도와 같은 검은 원, 그리고 오르반이라는 이름이 새겨져 있었다."),
        ),
        chapter(
            3,
            "종이 새가 물어온 이름",
            ...
            act("hunted_messenger", "쫓기는 전령", "{heroTopic} 편지를 되찾으려는 자들을 따돌리고 마지막 봉인까지 지켜냈다."),
        ...
        chapter(
            ...
            "문 없는 예배당",
            "세 새벽종에 감춰진 진짜 역할",
            "지도 속 예배당에는 창도 문도 없었고, 벽화에는 첫 새벽종을 울리라는 문구가 새겨져 있었다.",
            "오르반은 세 새벽종이 무기가 아니라 재의 문을 잠그는 열쇠라고 밝혔다. 세 종을 따로 울리면 문이 열리고 함께 울리면 새벽이 온다.",
            "남은 문제: 도둑맞은 두 새벽종을 되찾아야 한다.",
            act("wall_chapel", "벽뿐인 예배당", "지도 속 예배당에는 창도 문도 없었고, 벽화에는 첫 새벽종을 울리라는 문구가 새겨져 있었다."),
            act("clapper_door", "종소리가 여는 길", "첫 번째 새벽종을 울리자 벽이 갈라지며 오래 숨겨진 계단이 모습을 드러냈다."),
            act("three_dawn_bells", "세 개의 새벽종", "지하 벽화는 세 종이 국경을 지키는 무기가 아니라 재의 문을 잠그는 열쇠임을 보여주었다."),
            act("silent_hymn", "침묵의 찬가", "오르반은 소리 없는 찬가를 손끝으로 짚으며 누군가 남은 두 새벽종을 모두 훔쳤다고 털어놓았다."),
        ...
        chapter(
            ...
            "{heroWith} 길잡이 니아는 사라진 밀수꾼과 국경군 보급함을 함께 찾아냈다. 밀수로라 불린 길은 세 종탑과 성채를 잇는 군용 지하로였다.",
            ...
            act("customs_tunnel", "세관 아래의 길", "{heroTopic} 세관 바닥의 밀도를 두드려 군수품이 오간 비밀 통로를 찾아냈다."),
        ...
        chapter(
            ...
            act("borrowed_flag", "빌린 깃발", "{heroTopic} 적의 깃발을 들어 재의 무리를 유인하고 갇힌 생존자들을 빠져나오게 했다."),
        ...
        chapter(
            ...
            act("west_gate_names", "서문을 지킨 이름들", "구해 둔 로웬과 생존자들이 서문을 버티는 동안 {heroTopic} 무너진 종탑으로 향했다."),
        ...
        chapter(
            ...
            act("different_ink", "다른 잉크", "{heroTopic} 한 문서의 잉크에만 우물의 검은 재가 섞였음을 알아냈다."),
        ...
        chapter(
            ...
            "니아가 열어 준 밀수로를 통해 {heroTopic} 두 번째 새벽종을 되찾았다. 하지만 마지막 수레는 성채로 들어갔고, 한 번 울린 종이 하늘의 빛을 껐다.",
            "마지막 새벽종은 재의 성채 안에 있다.",
            ...
            act("second_clapper", "두 번째 새벽종", "{heroTopic} 전복된 수레에서 두 번째 새벽종을 되찾았지만 마지막 수레는 성채 문을 통과했다."),
        ...
        chapter(
            ...
            "베른은 왕도가 버린 국경을 지키려고 잠든 파수자를 깨우려 했다. 오르반의 경고에도 마지막 새벽종을 울리며, 버려진 국경에는 괴물이라도 왕이 필요하다고 말했다.",
            ...
            act("last_clapper", "마지막 새벽종", "베른은 경고를 듣고도 마지막 새벽종을 울렸다. ‘버려진 국경에는 괴물이라도 왕이 필요하다.’"),
        ),
        chapter(
            11,
            "새벽을 삼킨 왕",
            ...
            "국경에서 이어 온 모든 길이 사람들의 퇴로가 되었다. {heroSubject} 재의 왕을 깨뜨리자 베른은 처음으로 누구의 이름도 지우지 않은 철수 명령을 내렸다.",
            ...
            act("three_answers", "세 종의 답", "오르반은 흩어진 두 종소리를 한 박자로 묶었고 {heroTopic} 마지막 종까지 닿을 틈을 얻었다."),
        ...
        chapter(
            ...
            "사람들은 흩어진 세 새벽종을 모았고, 오르반은 서로 다른 상처를 가진 세 종의 음을 맞췄다.",
            "세 종이 함께 울리자 국경의 아침은 물 긷는 소리와 문 여는 소리로 돌아왔다. 베른이 남긴 유리 잎과 북쪽 숲의 지도는 끝난 이야기 너머의 길을 가리켰다.",
            "후일담: 국경의 새 삶과 북쪽에서 온 낯선 징조가 이어진다.",
            act("three_clappers", "되찾은 세 새벽종", "사람들은 흩어진 세 새벽종을 모았고, 오르반은 서로 다른 상처를 가진 세 종의 음을 맞췄다."),
            act("three_towers", "세 탑의 불빛", "로웬은 성벽, 니아는 지하로, {heroTopic} 성채의 종탑에 올라 같은 시각을 기다렸다."),
    ...
    val epilogues: List<AdventureTaleDefinition> = listOf(
        epilogue(
            ...
            "{heroTopic} {minorEnemy}의 둥지를 치우고 국경과 라움의 끊긴 길을 다시 이었다. 두 지역의 첫 공동 보급대가 같은 표지석을 지나갔다.",
            ...
            repeatAct("wheel_tracks", "바퀴자국", "{heroTopic} 굳은 진흙 아래 겹쳐진 오래된 바퀴자국을 따라갔다.", "덧씌운 자국을 걷어 내고 수레가 끌려간 샛길을 찾아냈다."),
            repeatAct("road_nest", "길목의 둥지", "{minorEnemySubject} 무너진 표지석 아래 둥지를 틀어 길을 막고 있었다.", "둥지를 치우고 빼앗긴 보급품을 온전히 되찾았다."),
        ...
        epilogue(
            ...
            "{heroTopic} {minorEnemyObject} 몰아내고 국경 전쟁 때 북방에 닿지 못한 구조 표식을 마엘과 로웬에게 전했다. 오래된 요청이 마침내 기록됐다.",
            "남은 소식: 기억나무의 순찰로가 기존 지도와 다른 방향으로 열렸다.",
            ...
            repeatAct("nameless_tag", "이름 없는 군패", "{minorEnemySubject} 지키던 틈에서 이름이 닳아 없어진 군패가 나왔다.", "군패의 부대 표식으로 돌아오지 못한 병사의 마지막 임무를 찾았다."),
            repeatAct("last_return", "마지막 귀환로", "{heroTopic} 지도에서 지워진 초소와 성문 사이의 길을 다시 걸었다.", "묻힌 표지석을 세우고 끊겼던 귀환로를 기록에 되돌렸다."),
        ...
        epilogue(
            ...
            "유리 잎이 가리킨 곳",
            "새로 열린 기억나무 공동 순찰로",
            "봄을 되찾은 기억나무의 유리 잎이 기존 지도에 없는 공동 순찰로를 비췄다.",
            "{heroTopic} {minorEnemy}의 흔적을 걷어 내고 기억나무가 새로 만든 길을 기록했다. 국경과 라움의 지도에 안전로 하나가 더해졌다.",
            "남은 소식: 공동 수로에서 재와 백야의 잔재가 함께 발견됐다.",
            repeatAct("lit_leaf", "빛을 머금은 잎", "기억나무의 유리 잎은 봄빛의 각도에 따라 지도에 없는 순찰로를 가리켰다.", "빛의 각도를 기록해 새로 열린 안전한 길을 계산했다."),
            ...
            repeatAct("three_replies", "세 번의 안전 답신", "{heroTopic} 종탑 거울로 라움에 약속한 짧은 빛을 보냈다.", "라움에서 세 번의 불빛이 돌아와 새 순찰로의 안전을 확인했다."),
        ),
        epilogue(
            4,
            "waterway",
            ...
            "{heroTopic} 물밑의 재와 유리 뿌리를 걷어 내고 두 지역의 공동 수로를 다시 연결했다. 흐르는 물이 같은 아침을 알렸다.",
        ...
        epilogue(
            ...
            "{heroTopic} 로웬과 마엘의 명부를 맞춰 이름 없는 묘표에 주인을 돌려주었다. 두 지역은 돌아오지 못한 사람을 하나의 기록에 남겼다.",
            ...
            repeatAct("patrol_keepsakes", "순찰대의 유품", "{minorEnemySubject} 끌어 모은 금속 조각 사이에 군패와 편지 고리가 섞여 있었다.", "유품을 되찾아 묘표와 명부의 이름을 하나씩 연결했다."),
        ...
        epilogue(
            ...
            "{heroTopic} {minorEnemySubject} 어지럽힌 신호를 바로잡고 공동 순찰대와 안전 기록을 교환했다. 열린 길은 정기 점검로로 등록됐다.",
            "남은 소식: 새로 연 길 아래에서 보급 수레 한 대가 흔적도 없이 사라졌다.",
            ...
            repeatAct("open_path", "지켜 낸 길", "기억나무가 바꾼 새 길을 정식 공동 순찰로로 남길지 점검했다.", "감시와 구조에 쓰는 공동 길로 확인하고 양쪽 표지석을 다시 세웠다."),
        ),
    ) + GuardianEpilogueCatalog.tales

    val prologues: List<AdventureTaleDefinition> get() = StarterPrologueCatalog.tales
    val labyrinths: List<AdventureTaleDefinition> get() = LabyrinthTaleCatalog.tales
    val all: List<AdventureTaleDefinition> = prologues + mainTales + epilogues + labyrinths
    ...
    fun nextDefinition(current: AdventureTaleDefinition, taleSequence: Long): AdventureTaleDefinition {
        ...
        return when (current.kind) {
            TaleKind.PROLOGUE, TaleKind.MAIN -> epilogues.first()
            TaleKind.EPILOGUE -> epilogues.getOrNull(current.chapterNumber) ?: labyrinths.first()
            TaleKind.LABYRINTH -> {
                val currentIndex = labyrinths.indexOfFirst { it.id == current.id }.coerceAtLeast(0)
                labyrinths[(currentIndex + 1) % labyrinths.size]
            }
        }
    }
    ...
    internal fun mainChapterTotalTarget(chapterNumber: Int): Long {
        val safeChapter = chapterNumber.coerceIn(1, MAIN_TALE_COUNT)
        return when (safeChapter) {
            1 -> 1_250L
            2 -> 3_000L
            else -> 5_000L + (safeChapter - 3L) * 100L
        }
    }

    internal fun mainActTarget(chapterNumber: Int, actIndex: Int): Long {
        val rhythmUnit = mainChapterTotalTarget(chapterNumber) / MAIN_ACT_RHYTHM_WEIGHT_TOTAL
        return rhythmUnit * MAIN_ACT_RHYTHM_WEIGHTS[actIndex.coerceIn(0, ACTS_PER_TALE - 1)]
    }
    ...
    internal fun targetFor(
        ...
        val definition = requireDefinition(definitionId)
        return if (definition.kind == TaleKind.LABYRINTH) {
            LabyrinthProgression.actTarget(labyrinthDepth, actIndex)
        } else {
            definition.acts[actIndex.coerceIn(0, ACTS_PER_TALE - 1)].target
        }
    }
    ...
    fun instantiate(
        ...
        val safeLabyrinthDepth = if (definition.kind == TaleKind.LABYRINTH) {
            labyrinthDepth.coerceAtLeast(1L)
        } else {
            0L
        }
        val isLabyrinthGate = definition.kind == TaleKind.LABYRINTH &&
            LabyrinthProgression.isGateDepth(safeLabyrinthDepth)
        val labyrinthGateNumber = if (isLabyrinthGate) {
            LabyrinthProgression.gateNumberForDepth(safeLabyrinthDepth)
        } else {
            0L
        }
        fun render(template: String): String = TOKEN_REGEX.replace(template) { match ->
            when (match.groupValues[1]) {
                "hero" -> heroName
                "heroTopic" -> withKoreanParticle(heroName, consonant = "은", vowel = "는")
                "heroSubject" -> withKoreanParticle(heroName, consonant = "이", vowel = "가")
                "heroWith" -> withKoreanParticle(heroName, consonant = "과", vowel = "와")
                "minorEnemy" -> variant.minorEnemy
                "minorEnemySubject" -> withKoreanParticle(
                    variant.minorEnemy,
                    consonant = "이",
                    vowel = "가",
                )
                "minorEnemyObject" -> withKoreanParticle(
                    variant.minorEnemy,
                    consonant = "을",
                    vowel = "를",
                )
                "depth" -> safeLabyrinthDepth.toString()
                else -> error("Unsupported tale token: ${match.value}")
            }
        }
        val labyrinthTier = if (heroLevel >= LABYRINTH_DEEP_LEVEL) "심층" else "표층"
        val renderedOpening = render(definition.openingTemplate) + if (isLabyrinthGate) {
            " 귀환 장부가 붉게 빛나며 제${labyrinthGateNumber}관문의 문지기가 구역 끝을 막았다."
        } else {
            ""
        }
        val renderedEnding = render(definition.endingTemplate) + if (isLabyrinthGate) {
            " 제${labyrinthGateNumber}관문 돌파 기록이 귀환 장부에 새겨졌고, " +
                "‘${LabyrinthProgression.titleForCompletedDepth(safeLabyrinthDepth)}’ 칭호가 해금됐다."
        } else {
            ""
        }
        return AdventureTaleState(
            ...
            volumeTitle = when {
                ...
                definition.kind == TaleKind.LABYRINTH -> "${labyrinthTier} 미궁"
                definition.volumeNumber == 1 -> "잿빛 국경"
                else -> "유리 숲의 백야"
            },
            chapterNumber = if (definition.kind == TaleKind.LABYRINTH) {
                safeLabyrinthDepth.coerceAtMost(Int.MAX_VALUE.toLong()).toInt()
            } else {
                definition.chapterNumber
            },
            title = when (definition.kind) {
                TaleKind.PROLOGUE, TaleKind.MAIN -> definition.title
                TaleKind.EPILOGUE -> definition.title
                TaleKind.LABYRINTH -> if (isLabyrinthGate) {
                    "제${safeLabyrinthDepth}구역 · 제${labyrinthGateNumber}관문 · ${definition.title}"
                } else {
                    "제${safeLabyrinthDepth}구역 · ${definition.title}"
                }
            },
            subtitle = when (definition.kind) {
                TaleKind.PROLOGUE, TaleKind.MAIN -> definition.subtitle
                TaleKind.EPILOGUE -> definition.subtitle
                TaleKind.LABYRINTH -> "${labyrinthTier} ${if (isLabyrinthGate) "관문" else "원정"} · " +
                    definition.subtitle
            },
            opening = renderedOpening,
            ending = renderedEnding,
            nextHook = render(definition.nextHookTemplate),
            variant = variant,
            acts = definition.acts.mapIndexed { index, act ->
                val baseRewardExperience = scalePacingTarget(
                    safeAdd(24L, safeMul(heroLevel, 8L)),
                )
                val baseRewardGold = safeAdd(12L, safeMul(heroLevel, 3L))
                TaleActState(
                    id = act.id,
                    number = index + 1,
                    title = if (isLabyrinthGate && index == definition.acts.lastIndex) {
                        "관문 · ${act.titleTemplate}"
                    } else {
                        act.titleTemplate
                    },
                    ...
                    progress = 0L,
                    target = if (definition.kind == TaleKind.LABYRINTH) {
                        LabyrinthProgression.actTarget(safeLabyrinthDepth, index)
                    } else {
                        act.target
                    },
                    rewardExperience = if (definition.kind == TaleKind.LABYRINTH) {
                        LabyrinthProgression.scaleReward(baseRewardExperience, safeLabyrinthDepth)
                    } else {
                        baseRewardExperience
                    },
                    rewardGold = if (definition.kind == TaleKind.LABYRINTH) {
                        LabyrinthProgression.scaleReward(baseRewardGold, safeLabyrinthDepth)
                    } else {
                        baseRewardGold
                    },
                )
            }.toMutableList(),
            openingSlides = definition.openingSlideTemplates.map(::render),
            labyrinthDepth = safeLabyrinthDepth,
        )
    }

    fun validationErrors(): List<String> = buildList {
        if (mainTales.size != MAIN_TALE_COUNT) add("main tale count must be $MAIN_TALE_COUNT")
        if (prologues.size != HeroClass.entries.size) add("one prologue per class is required")
        if (epilogues.size != EPILOGUE_TALE_COUNT) add("epilogue count must be $EPILOGUE_TALE_COUNT")
        if (epilogues.map { it.chapterNumber } != (1..EPILOGUE_TALE_COUNT).toList()) {
            add("epilogue chapter order is invalid")
        }
        if (labyrinths.isEmpty()) add("labyrinth templates must not be empty")
        if (labyrinths.any { it.kind != TaleKind.LABYRINTH }) add("invalid labyrinth kind")
    ...
    private fun withKoreanParticle(
        value: String,
        consonant: String,
        vowel: String,
    ): String {
        val last = value.trimEnd().lastOrNull()
        val hasFinalConsonant = last != null && last in '\uAC00'..'\uD7A3' &&
            (last.code - HANGUL_SYLLABLE_START) % HANGUL_FINAL_CONSONANT_COUNT != 0
        return value + if (hasFinalConsonant) consonant else vowel
    }

    const val EPILOGUE_TALE_COUNT = 18
    const val LABYRINTH_DEEP_LEVEL = 100L

    private const val MAIN_ACT_RHYTHM_WEIGHT_TOTAL = 25L
