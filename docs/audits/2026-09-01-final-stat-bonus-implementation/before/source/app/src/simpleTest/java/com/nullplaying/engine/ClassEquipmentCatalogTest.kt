...
class ClassEquipmentCatalogTest {
    ...
    fun `class labels and class equipment use the approved terminology`() {
        ...
        assertTrue(SimpleContent.equipmentBases(EquipmentSlot.HEAD, 1L, HeroClass.RANGER).first().contains("레인저"))
    ...
    fun `legacy class equipment terms modernize without touching narrative words`() {
        ...
        assertEquals("모험식 레인저 창", ClassEquipmentCatalog.modernizeName("모험식 순찰자 창"))
        assertEquals("모험식 레인저 창", ClassEquipmentCatalog.modernizeName("모험식 레인져 창"))
