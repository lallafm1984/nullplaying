...
object StatBonusEngineProbe {
    ...
    fun main(args: Array<String>) {
        ...
        val engine = SimpleGameEngine(OfflineAdventureConfig(480, 20))
        output.bufferedWriter().use { writer ->
            writer.appendLine("class,seed_index,level,str,con,dex,int,wis,cha,hp,mp,tales,kills,seconds,bag,proc_pct,skill_damage_pct,skills,skill_uses,stat_power,equipment_power,sale_gold,items_sold,attacks,casts,experience,bonus_cap_h,bonus_proc_pct,bonus_search_s,bonus_sale_pct")
                    ...
                    while (true) {
                        if ((dense || state.hero.level in levels) && state.hero.level != recordedLevel) {
                            ...
                            writer.appendLine(listOf(heroClass.name, index, h.level,
                                ...
                                engine.baseSkillProcPercent(state), skillDamage, state.skills.size,
                                ...
                                BalanceSimulationHooks.casts, h.experience,
...
StatBonusRules.encounterSearchMillis(state)/1000.0, StatBonusRules.saleBonusPercent(state)).joinToString(","))
    ...
    private fun verifyBankInvariants() {
        for (capacity in listOf(480L, 540L, 600L)) {
            ...
            check(state.offlineAdventureMillis == engine.offlineAdventureCapacityMillis(state) / 12)
            engine.advanceOfflineAdventureForeground(state, 11 * 60_000L)
            check(state.offlineAdventureMillis == engine.offlineAdventureCapacityMillis(state))
